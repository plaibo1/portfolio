...
export const PrimaryButton = ({ text, linkTo, icon = null }) => {
  return (
    <Link href={linkTo}>
      ...
      >
        <div
          className="w-[102%] h-[115%] absolute bg-slate-700 left-0 top-0 rounded-xl 
        border-slate-700 border-4 -z-[1] -translate-x-3 -translate-y-2 
        group-hover:translate-x-0 group-hover:translate-y-0 transition"
        ></div>
...
export const ProjectLinkButton = ({ linkTo }) => {
  return (
    ...
    >
      <div
        className="w-[105%] h-[110%] absolute bg-slate-800 left-0 top-0 rounded-xl 
...
export const LinkButtonFire = ({ linkTo, name, icon }) => {
  return (
    ...
    >
      <div
        className={`absolute left-1/2 -translate-x-[50%] w-full -top-[80px] sm:-top-[99px] h-[95px] sm:h-[120px]
            group-hover:opacity-100 group-hover:block pointer-events-none opacity-0
           `}
      >
        <Image layout="fill" src={"/speed.gif"} alt="on fire!!!" />
      </div>
      {name} {icon}
    </a>
  );
...
export const ButtonFire = ({ click, name, icon }) => {
  return (
    <button
      onClick={click}
      className="text-baseColor py-2 px-5 rounded-lg w-full
             border-2 border-dashed relative group text-sm sm:text-base
             justify-between flex items-center font-mono
             hover:bg-baseColor hover:border-solid hover:border-baseColor hover:text-white"
    >
      <div
        className={`absolute left-1/2 -translate-x-[50%] w-full -top-[80px] sm:-top-[99px] h-[95px] sm:h-[120px]
           ...
           `}
      >
        <Image layout="fill" src={"/speed.gif"} alt="boom boom" />
      </div>
      {name} {icon}
    </button>
  );
